...
export function TeamCreateModal({ onClose, onSave }: TeamCreateModalProps) {
  ...
  const validateName = async (name: string) => {
    ...
    try {
      const isValid = await teamService.validateTeamName(name)
      if (!isValid) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Fel vid skapande av team')
    } finally {
      setLoading(false)
    }
  }

  const isValid = formData.name.trim() && !nameError && !loading
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            >
              ...
              <span>{loading ? 'Skapar...' : 'Skapa Team'}</span>
            </button>
          </div>
        </form>
      </div>
